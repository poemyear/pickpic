...
export default class SwitchButton extends Component {
    ...
    _switchThump(direction) {
        ...
        if (this.state.activeSwitch === 0){
            ...
            this.setState({ activeSwitch: 0 }, () => onValueChange(this.state.activeSwitch));
            Animated.timing(
                this.state.offsetX,
                    {
                        toValue: 0,
                ...
                <TouchableOpacity activeOpacity={1} onPress={ () => { this._switchThump(this.props.switchdirection || this.state.direction) } }>
                    ...
                    >
                            <View style={[{ flexDirection: this._switchDirection(this.props.switchdirection || this.state.direction) }]} >
                                ...
                                >
                                    <Text style={[ this.state.activeSwitch === 0 ? { color: this.props.activeFontColor || "#fff" } : { color: this.props.fontColor || "#b1b1b1" }  ]}>
                                        { this.props.text1 || 'ON' }
                                    </Text>
                                </View>
